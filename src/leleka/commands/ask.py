...
def ask_cmd(
    ...
    with llm_helpers.create_streaming_panel("", title=f"Leleka ({model})", title_color="magenta", border_color="cyan") as live:
        for chunk in response_stream:
            ...
            if chunk.get('done'):
                stats = llm_helpers.calc_token_stats(chunk)
                llm_helpers.show_context_stats(stats)